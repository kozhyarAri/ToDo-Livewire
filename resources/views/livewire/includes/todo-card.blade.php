<div wire:key='{{ $todo->id }}'
    class="col-span-1 px-5 py-6 mb-5 bg-white border-t-2 border-blue-500 rounded-md shadow todo card hover:shadow-md">
    <div class="flex justify-between space-x-2">
        ...
        <div class="flex items-center space-x-2">
            ...
            <button wire:confirm='Are you sure?' wire:click='delete({{ $todo->id }})'
    ...
    <span class="text-xs text-gray-500 bg"> {{ $todo->created_at }} </span>
    @if ($todo->image)
        <img class="h-16" src="{{ asset('storage/' . $todo->image) }}" alt="">
    @endif
